...
async function handleLogin(e) {
    ...
    try {
        ...
        if (user) {
            ...
            // Update cart session for logged-in user
            localStorage.setItem('cartOwnerSession', 'user_' + (user.id || user.email));
            updateAuthUI();
...
async function handleSignup(e) {
    ...
    try {
        ...
        if (user) {
            currentUser = user;
            localStorage.setItem('currentUser', JSON.stringify(user));
            // Update cart session for newly created account
            localStorage.setItem('cartOwnerSession', 'user_' + (user.id || user.email));
            updateAuthUI();
...
function showAuthNotification(message, type) {
    ...
    // All notifications in yellow theme
...
function updateAuthUI() {
    ...
    // Update mobile account dropdown if present
    const mobileAccountMenu = document.getElementById('mobile-account-menu');
    if (mobileAccountMenu) {
        mobileAccountMenu.innerHTML = '';
        if (currentUser) {
            mobileAccountMenu.innerHTML = `
                <a href="#" class="mobile-dropdown-item" id="mobile-account-logout"><i class="fas fa-sign-out-alt"></i> Logout</a>
                <a href="#" class="mobile-dropdown-item"><i class="fas fa-user"></i> ${currentUser.firstName}</a>
            `;
            // Attach logout handler
            const logoutEl = document.getElementById('mobile-account-logout');
            if (logoutEl) logoutEl.addEventListener('click', (e) => { e.preventDefault(); logout(); });
        } else {
            mobileAccountMenu.innerHTML = `
                <a href="login.html" class="mobile-dropdown-item"><i class="fas fa-sign-in-alt"></i> Sign In</a>
                <a href="login.html" class="mobile-dropdown-item"><i class="fas fa-user-plus"></i> Create Account</a>
            `;
        }
    }
...
function logout() {
    ...
    localStorage.removeItem('cartOwnerSession');  // Clear session ownership
    sessionStorage.removeItem('sessionId');  // Clear session ID for new anonymous user
    updateAuthUI();
