...
const transporter = nodemailer.createTransport({
    service: 'gmail',
    auth: {
        user: process.env.GMAIL_USER || 'rnakiyemba@student.refactory.academy',
...
async function sendSubscribeEmail(subscriberEmail) {
    try {
        const mailOptions = {
            from: 'rnakiyemba@student.refactory.academy',
            to: 'rnakiyemba@student.refactory.academy',
...
async function sendPasswordResetEmail(userEmail, resetCode) {
    try {
        ...
        const resetLink = `http://localhost:${port}/reset-password.html?code=${resetCode}&email=${encodeURIComponent(userEmail)}`;
        const mailOptions = {
            from: 'rnakiyemba@student.refactory.academy',
            to: userEmail,
