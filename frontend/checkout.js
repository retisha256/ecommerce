...
function loadCartToCheckout() {
    ...
    orderItemsContainer.innerHTML = cart.map(item => {
        const priceValue = typeof item.price === 'number' ? item.price : parseFloat(String(item.price).replace(/[^\d]/g, '')) || 0;
...
function setupCheckoutForm() {
    ...
    form.addEventListener('submit', function (e) {
        ...
        const orderData = {
            ...
            paymentMethod: formData.get('payment'), // Changed from 'payment' to matches backend schema
            items: cart.map(item => ({
                productId: item._id || item.id, // Ensure we send productId
                name: item.name,
                category: item.category,
                price: typeof item._priceValue === 'number' ? item._priceValue : (typeof item.price === 'number' ? item.price : parseFloat(String(item.price).replace(/[^\d]/g, '')) || 0),
...
async function processOrder(orderData) {
    ...
    try {
        ...
        // Save order to backend API immediately
        if (typeof api !== 'undefined') {
            await api.createOrder(orderData);
            console.log('Order created successfully:', paymentData.orderId);
        } else {
...
async function confirmPayment(orderId) {
    ...
    if (orderData && paymentData) {
        try {
            // Verify payment on backend API
            if (typeof api !== 'undefined') {
                await api.verifyPayment({
                    orderId: orderId,
                    paymentReference: paymentData.paymentReference || 'mobile_money_confirm'
                });
            } else {
